...
Application Gate v1 corrected (`APPLICATION_GATE_V1_CORRECTED`) = **IMPLEMENTED — PENDING INDEPENDENT REAUDIT** (adds Gate 1.5 representation/evaluation of real application-form questions, separate from public JD qualification: `Job → ApplicationAttempt → ApplicationQuestion → ApplicationAnswer`, plus derived `ApplicationQuestionEvaluation`; four new schemas (`application_attempt`, `application_question`, `application_answer`, `application_question_evaluation`) and four new `src/` modules (`application_logic.py` deterministic three-valued ALL_OF/ANY_OF/AT_LEAST_N/NOT evaluation, `application_clause_match.py` a minimal adapter reusing Gate-1's Evidence/Claim capability matcher for application-only clauses without ever creating a JD Requirement, `application_answer.py` exploratory/intended/submitted answer separation, `application_gate.py` the per-question evaluator plus the Gate-0 short-circuit predicate); `filter_risk` deliberately supports only `UNKNOWN`/`POTENTIAL_KNOCKOUT` (no `ACTUAL_KNOCKOUT_CONFIGURED`); `answer_policy=ALWAYS_HUMAN` forces `safe_boolean_answer=NOT_APPLICABLE` regardless of predicate_result; ApplicationQuestion source records are never mutated by reevaluation; does not change `job_decision.py`, lane vocabulary, existing Gate-1 routing, résumé derivative selection, or submission logic; six new Golden/adversarial cases (`GT_APP_GATE_CLEAN`, `GT_APP_GATE_COMPOUND_UNSAFE`, `GT_APP_GATE_EXPLORATORY_ISOLATED`, `GT_APP_GATE_GATE0_SHORT_CIRCUIT`, `GT_APP_GATE_FORM_ONLY_CLAUSE`, `GT_APP_GATE_REEVALUATION_NO_SOURCE_MUTATION`) plus targeted unit tests. **F-01/F-02 remediation (`APPLICATION_GATE_V1_EVALUATION_INPUT_DIGEST_REMEDIATION`) implemented locally, pending independent reaudit**: independent Cursor audit found the recorded evaluation digest (`evidence_version`) was derived only from `evidence_index`, while `evaluate_application_question()` also depends semantically on `claim_index` via `load_reusable_claims()` — a Claim-only change (wording/evidence_state/evidence_ids/human_approval) could change the evaluation result while the recorded digest stayed identical (F-01, MEDIUM, audit-misleading); `GT_APP_GATE_REEVALUATION_NO_SOURCE_MUTATION`'s OR-condition assertion could also pass without the digest actually changing (F-02, LOW). Fixed by renaming the field to `evaluation_inputs_digest` and computing one combined SHA-256 digest over canonical JSON of `{evidence_index, claim_index}` (both trusted indexes hashed in full, not a hand-selected subset of Claim fields); `GT_APP_GATE_REEVALUATION_NO_SOURCE_MUTATION` now asserts `evaluation_a["evaluation_inputs_digest"] != evaluation_b["evaluation_inputs_digest"]` directly, with no OR fallback; four new targeted digest tests (`tests/application_evaluation_digest_test.py`) prove same-Evidence+same-Claims→same digest, Evidence-only change→different digest, Claims-only change→different digest, and insertion-order-only difference→same digest. No ApplicationAttempt/ApplicationQuestion/ApplicationAnswer schema, clause structure, logic operator, truth/support mapping, ALWAYS_HUMAN behavior, filter_risk, screening_materiality, or `gate_1_5_applicable` behavior changed. 42/42 repository test suites pass (41 prior + 1 new); 15/15 Golden pass with zero routing/outcome drift; no independent Cursor reaudit of this remediation has occurred yet; see below).
...
`APPLICATION_GATE_V1_CORRECTED` plus its `APPLICATION_GATE_V1_EVALUATION_INPUT_DIGEST_REMEDIATION` (F-01/F-02) fix are both implemented locally and pending independent Cursor reaudit (not yet closed, not yet pushed). No further Application Gate work, no Bulmarma, no D Commerce Bank, no PDF/DOCX, no Summary, no real résumé generation, no job-specific tailoring without separate approval.
