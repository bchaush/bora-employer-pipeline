...
`TELUS_EVIDENCE_V1` is implemented pending independent Cursor re-audit; not pushed. No Claims, résumé modules, or master integration for TELUS yet (a future, separately-scoped `TELUS_RESUME_MODULES_V1` milestone would follow after this Evidence passes audit). No Bulmarma, no D Commerce Bank, no PDF/DOCX, no Summary, no real résumé generation, no job-specific tailoring begun.
...
* Updated hardcoded repository-count assertions (Experience 3→4, Evidence 29→36) across 16 existing test files and `golden-tests/run_job_analysis_golden_set.py`'s own baseline regression check — a count-baseline correction only; all 15 individual Golden fixture routing outcomes unchanged.
