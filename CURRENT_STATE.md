...
## Current Execution Checkpoint — SUPERVISED_PRODUCTION_V1 (2026-09-26)

`SUPERVISED_PRODUCTION_V1` remains the accepted active production doctrine. Slice 1 and Slice 2 remain **COMPLETED_BY_OPERATOR / BORA_ACCEPTED**. Production run `PROD_20260926_FIRST_REAL_BATCH_001` provided the next architectural evidence: five real nominations correctly failed closed to five durable `VERIFICATION_REQUIRED` LOG rows, zero JOBS rows, and an exact rerun produced zero new mutations. Current first-party verification resolves MTA to employer Job ID `17407`, Peraton to requisition `2026-171010`, and OneMain to Job Number `R2608-52225`; Middesk and Runlayer remain unresolved for exact employer requisition/opportunity identity.

`SUPERVISED_PRODUCTION_V1_FIRST_PARTY_IDENTITY_RESOLUTION_V1` is now the next candidate seam and is **SELECTED_NOT_AUTHORIZED**. Frozen runtime contract: `milestone_contracts/governance/supervised-production-v1-first-party-identity-resolution-v1-contract.json`. Global `implementation_authorized=false`. The exact next allowed action is governance only: validate, independently review, canonically merge, and recover the frozen contract/routing state, then obtain Bora's separate explicit runtime authorization. No runtime resolver implementation, Match Truth redesign, package work, Vercel, Scout expansion, browser automation, auto-submit, PURSUE persistence, or database work is authorized by this selection.
...
`CAREER_OS_EVAL_AND_HARNESS_AUDIT_V1` (Phase B) and `CAREER_OS_TRACE_AND_FAILURE_CORPUS_INVENTORY_V1` (Phase C) were both completed by the operator in READ-ONLY mode and have both been **BORA ACCEPTED**. The governance-only `CAREER_OS_AGENT_CONTEXT_AND_USAGE_EFFICIENCY_V1` policy milestone, drafted at `docs/decisions/ADR-CAREER-OS-AGENT-CONTEXT-USAGE-EFFICIENCY-V1.md`, remains **COMPLETED_BY_OPERATOR / BORA_ACCEPTED (2026-09-11) / GOVERNANCE_ONLY / GOVERNING**, unaffected by the transition below. `CAREER_OS_FAILURE_TAXONOMY_AND_EVALUATOR_COVERAGE_MAP_V1` (Phase D) substantive read-only/design-only work remains **COMPLETED_BY_OPERATOR / BORA_ACCEPTED (2026-09-11) / READ_ONLY_DESIGN_ONLY**, rendered at `docs/audits/CAREER_OS_FAILURE_TAXONOMY_AND_EVALUATOR_COVERAGE_MAP_V1_REPORT.md`, and has now dropped out of the live three-slot prior_phase/prior_prior_phase/prior_prior_prior_phase chain (as Phase C did before it), preserved unchanged as historical reference. `CAREER_OS_SYSTEM_EVAL_SET_ARCHITECTURE_V1` (Phase E) substantive read-only/design-only work is **COMPLETED_BY_OPERATOR / BORA_ACCEPTED (2026-09-12) / READ_ONLY_DESIGN_ONLY**, rendered at `docs/audits/CAREER_OS_SYSTEM_EVAL_SET_ARCHITECTURE_V1_REPORT.md`, now recorded as prior_prior_prior_phase. `CAREER_OS_TRACE_AND_CONTRACT_ARCHITECTURE_V1` (Phase F) substantive conceptual trace/contract architecture work was completed by the operator, rendered at `docs/audits/CAREER_OS_TRACE_AND_CONTRACT_ARCHITECTURE_V1_REPORT.md`, and is now **COMPLETED_BY_OPERATOR / BORA_ACCEPTED (2026-09-14) / READ_ONLY_DESIGN_ONLY**, now recorded as prior_prior_phase: `CAREER_OS_RUN_TRACE_V1` defined conceptually only (no schema file, no implementation), the three-way terminal_state / first_point_of_divergence / first_causal_failure_point distinction (terminal_state = the outcome the run actually reached; first_point_of_divergence = the earliest identifiable divergence from correctness in the run's own trajectory, including one a later gate catches and recovers from; first_causal_failure_point = the divergence point -- that same earliest divergence, or a distinct, later one -- after which no later gate in the run's own trajectory recovered, making the incorrect terminal_state unavoidable, and equals first_point_of_divergence only when no earlier recovered divergence exists in that run; a run can contain an earlier recovered divergence A followed by a distinct, later, unrecovered divergence B, yielding first_point_of_divergence = A and first_causal_failure_point = B at two different points in the same run; a run can also have a recovered first_point_of_divergence = A whose actual causal defect lived in a distinct, later, uninstrumented boundary with no identifiable point B, in which case first_point_of_divergence = A is preserved unchanged (never wiped or reattributed) and first_causal_failure_point is separately recorded as CAUSE_UNKNOWN / UNINSTRUMENTED_BOUNDARY rather than a fabricated point B, distinct from the fully-uninstrumented case where both fields carry that same symmetric CAUSE_UNKNOWN / UNINSTRUMENTED_BOUNDARY label; a correct terminal_state with no divergence records first_point_of_divergence as NOT_APPLICABLE / NO_DIVERGENCE, and any correct terminal_state -- including one preserving a recovered first_point_of_divergence = A unchanged -- records first_causal_failure_point as NOT_APPLICABLE / NO_TERMINAL_FAILURE, never CAUSE_UNKNOWN / UNINSTRUMENTED_BOUNDARY, which remains reserved exclusively for an incorrect terminal_state with a real but uninstrumented divergence or causal failure), the human-handoff architecture, the bounded phase/typed-envelope contract, the append-only-facts-vs-derived-summaries distinction, and artifact-hash/model-provenance discipline (Bora explicitly stated, on 2026-09-14: "I explicitly accept Phase F brother"). Bora, before that Phase F acceptance closed, explicitly stated: "once we are proper to standard and all u have my absolute authorization to start the next phase G" -- a conditional authorization of Phase G tied to canonical main reaching proper standard, satisfied because canonical main stood at `e199623c73b08e986beb32437e65f40a5b63b435` at authorization time. Bora, after PR #57 merged, explicitly confirmed: "done G lets keep going brother we got this beautiful work we are doing here." Phase G's substantive two-loop assurance architecture design -- FOCUSED VALIDATION / ITERATION DIAGNOSTIC (milestone-scoped, explicitly NON_AUTHORITATIVE) separated from FULL ASSURANCE (`python scripts/verify_assurance_baseline.py`, the sole promotion-authoritative full-suite gate, unmodified) -- is rendered at `docs/audits/CAREER_OS_ASSURANCE_ARCHITECTURE_V1_REPORT.md`, grounded in the existing canonical assurance runtime plus one round of OBSERVED single-sample local timing evidence (89 standalone tests, 416.406s total; `milestone_run_v1_test.py` alone 318.741s) and one separate hosted GitHub Actions observation (run `34915038186`, ~47s verify job), both explicitly labeled diagnostic evidence, never an SLA or reliability claim; no second canonical fast-assurance runner or global slow-test exclusion list was created; Phase G also ranked three Phase H recommendations, none implemented at Phase G's own completion: (A) timing observability; (B) bounded optimization of `milestone_run_v1_test.py` if repeated measurement continues to show it dominating; (C) a named `NON_AUTHORITATIVE_FOCUSED_DIAGNOSTIC` fast runner only after A/B. Bora, after PR #59 merged, independently verified (canonical main `b48b9a504836a1e502af96f77cd1f6fe947cb604`, reviewed head `d4c4368754c389c39c34f08990027a875ec7c443` confirmed as its parent/ancestor, hosted Assurance passed) and explicitly stated: "beautiful work G once u cehck everything being up to standart to the three guys we refer to u have all authorization moving forwrad, please G just mind the usage limits of Cursor and Claude love u bro" -- a genuine conditional human event, never self-granted by the operator. This single statement recorded two distinct governance facts, never conflated: (1) `CAREER_OS_ASSURANCE_ARCHITECTURE_V1` (Phase G) moved to **COMPLETED_BY_OPERATOR / BORA_ACCEPTED (2026-09-14) / READ_ONLY_DESIGN_ONLY**, now recorded as prior_phase, its substantive report preserved unchanged except status/acceptance framing; (2) the same statement separately authorized exactly one narrow bounded Phase H implementation milestone, `CAREER_OS_ASSURANCE_TIMING_OBSERVABILITY_V1`, selected from Phase G's own Recommendation A only (standardized diagnostic per-phase/per-test timing observability added to the existing `scripts/verify_assurance_baseline.py` path only, without changing pass/fail semantics, test discovery, ordering, mandatory coverage anchors, golden coverage, the hosted CI command, promotion authority, or terminal canonical success semantics), then BORA_AUTHORIZED / SELECTED / NOT_YET_COMPLETED, with implementation authority explicitly SCOPED_TO_THIS_MILESTONE_ONLY_NOT_GLOBAL. Phase H's substantive implementation (PR #61, `feature/career-os-phase-h-assurance-timing-observability-v1`) added that standardized diagnostic per-phase/per-test timing observability to `scripts/verify_assurance_baseline.py`, preserving all existing pass/fail semantics, discovery, ordering, mandatory coverage anchors, golden coverage, hosted CI command, promotion authority, and terminal canonical success semantics. Independently verified: PR #61 merged to canonical main `52227f2d2ac913768bc558619f38d507358ebbaa`, reviewed head `dc8cde625ec4308a3a9790ad907009684955a3ca` confirmed as its parent/ancestor, merge tree `c62db7f169c46c38b8df7dd7618a718e43d1525d` matching the reviewed tree, hosted post-merge Assurance successful. Bora, after PR #61 merged, explicitly stated on 2026-09-15: "Done brother carry the same discipline and manner and u have my authorization to move forward ofc if all is up to the standard of the three guys we refer to love u" -- a genuine conditional human event, never self-granted by the operator, whose condition the operator independently verified against canonical Git/GitHub and the Phase G Recommendation B evidence threshold. This single statement records two distinct governance facts, never conflated: (1) `CAREER_OS_ASSURANCE_TIMING_OBSERVABILITY_V1` (Phase H) is now **COMPLETED_BY_OPERATOR / BORA_ACCEPTED (2026-09-15) / BOUNDED_IMPLEMENTATION**, its substantive implementation preserved unchanged except status/acceptance framing; (2) the same statement historically authorized exactly one narrow bounded Phase H Recommendation B milestone, `CAREER_OS_MILESTONE_RUN_V1_TARGETED_OPTIMIZATION_V1`, selected from Phase G's own Recommendation B only -- bounded optimization of `tests/milestone_run_v1_test.py` or its disposable-real-Git/faked-provider fixture execution, preserving adversarial scenario coverage and causal invariants -- at that authorization time recorded as **BORA_AUTHORIZED / SELECTED / NOT_YET_COMPLETED**, with implementation authority explicitly SCOPED_TO_THIS_MILESTONE_ONLY_NOT_GLOBAL -- never a blanket/global implementation grant; that scoped authority is now exhausted by operator completion. The exact cross-environment timing evidence earning Recommendation B: local run 1 `milestone_run_v1_test.py`=318.025429s vs Phase 2=415.069786s; local run 2=312.284460s vs Phase 2=406.363878s; hosted post-merge run=12.904488s vs Phase 2=29.252603s; `milestone_run_v1_test.py` is the dominant individual Phase-2 test in all three observations. Recommendation C (a named `NON_AUTHORITATIVE_FOCUSED_DIAGNOSTIC` fast runner) remains **NOT_AUTHORIZED**; Recommendation B's optimization work has completed and does not open Recommendation C. `CURRENT_EXECUTION_CHECKPOINT.json` is the machine-readable fresh-chat handoff. `implementation_authorized` (the global/blanket flag) remains `false`. PR #63 has since merged the exact Recommendation B implementation to canonical main `0120bcde46a10f1ee7d5ec33e17f5fca7b8fc310` and has been independently verified for parent/ancestry, reviewed-tree equality, and hosted post-merge Assurance SUCCESS. Bora has now explicitly accepted Recommendation B, so it is **COMPLETED_BY_OPERATOR / BORA_ACCEPTED (2026-09-15)**. At that historical checkpoint, the next activity was **OPERATE FIRST / BUILD SECOND** using Career OS Personal v1 on real opportunities; no later engineering milestone was selected or authorized by that acceptance. Recommendation C remains **NOT_AUTHORIZED** and Phase I and every later roadmap phase remain **PROPOSED_NOT_AUTHORIZED**.

## Eval & Harness Audit Roadmap Reference (2026-09-15)

`CAREER_OS_EVAL_AND_HARNESS_AUDIT_V1` is **COMPLETED_BY_OPERATOR / BORA_ACCEPTED** (see the Historical Execution Checkpoint section above). `CAREER_OS_TRACE_AND_FAILURE_CORPUS_INVENTORY_V1` is **COMPLETED_BY_OPERATOR / BORA_ACCEPTED**. The governance-only `CAREER_OS_AGENT_CONTEXT_AND_USAGE_EFFICIENCY_V1` policy milestone remains **COMPLETED_BY_OPERATOR / BORA_ACCEPTED (2026-09-11) / GOVERNANCE_ONLY / GOVERNING**. Phase D (`CAREER_OS_FAILURE_TAXONOMY_AND_EVALUATOR_COVERAGE_MAP_V1`) is **COMPLETED_BY_OPERATOR / BORA_ACCEPTED (2026-09-11) / READ_ONLY_DESIGN_ONLY** (now historical reference only, dropped out of the live prior-phase chain). Phase E (`CAREER_OS_SYSTEM_EVAL_SET_ARCHITECTURE_V1`) is **COMPLETED_BY_OPERATOR / BORA_ACCEPTED (2026-09-12) / READ_ONLY_DESIGN_ONLY** (now prior_prior_prior_phase). Phase F (`CAREER_OS_TRACE_AND_CONTRACT_ARCHITECTURE_V1`) is **COMPLETED_BY_OPERATOR / BORA_ACCEPTED (2026-09-14) / READ_ONLY_DESIGN_ONLY** (now prior_prior_phase). Phase G (`CAREER_OS_ASSURANCE_ARCHITECTURE_V1`) is **COMPLETED_BY_OPERATOR / BORA_ACCEPTED (2026-09-14) / READ_ONLY_DESIGN_ONLY** (now prior_phase). Phase H (`CAREER_OS_ASSURANCE_TIMING_OBSERVABILITY_V1`) is **COMPLETED_BY_OPERATOR / BORA_ACCEPTED (2026-09-15) / BOUNDED_IMPLEMENTATION**. `CAREER_OS_MILESTONE_RUN_V1_TARGETED_OPTIMIZATION_V1` (Phase H Recommendation B) is the current record: **BORA_AUTHORIZED / SELECTED / COMPLETED_BY_OPERATOR / BORA_ACCEPTED (2026-09-15)**; its prior SCOPED_TO_THIS_MILESTONE_ONLY_NOT_GLOBAL implementation authority is exhausted and closed. The controlling sequence is `docs/decisions/ADR-CAREER-OS-EVAL-HARNESS-SEQUENCE-V1.md`; the governing context/usage-efficiency policy is recorded at `docs/decisions/ADR-CAREER-OS-AGENT-CONTEXT-USAGE-EFFICIENCY-V1.md`. The exhausted historical scoped authorization does not authorize trace infrastructure, a database/UI, orchestration automation, new agents, model/provider routing, LLM judges, or a context-surface structural refactor. Recommendation C remains **NOT_AUTHORIZED**. Phase I and every later roadmap phase remain **PROPOSED_NOT_AUTHORIZED**; no Recommendation-B implementation authority remains open, and no successor implementation authority exists. Fresh sessions recover in the single canonical order fixed by the ADR's Section 6 (New-chat recovery protocol) — this file does not restate a competing order.
...
(canonical merge commit `784856ca2fdeed562cac5c1ef8d24c5483549ac6`, PR #7,
parents `07321f12018980fcc2a392d5590a959fcf1941c1`/
`24a1c0829e27c662d6b31496a7ed80d611695dd6`; hosted Assurance Baseline run
`33784508138` on the merge commit independently verified `success`;
documentation/governance only -- `BLUEPRINT.md`, `AGENTS.md`, `CLAUDE.md`,
`CHANGELOG.md` only, no production/schema/test/fixture/Claim/Evidence/
Experience/résumé changes). Bumped `BLUEPRINT.md` v3.3 -> v3.4 and locked
new Section 133: operate-while-building; the build-economy gate (justified
only by (A) a demonstrated material reliability/truth defect or (B) a
material reduction in Bora's live search/application workload -- see
`BLUEPRINT.md` §133 for the full, canonical doctrine text rather than
duplicating it here); the HARD CORRECTION / CALIBRATION SIGNAL / ANECDOTE
real-world calibration tiers; the outcomes-do-not-rewrite-truth boundary;
the fresh first-party employer source rule; the conceptual, not-
implemented `DiscoveryLead` boundary; the commodity tool/adapter rule
(six owned Truth layers -- Employer/Candidate/Match/Pursuit/Package/
Outcome -- with replaceable external adapters around them); and a human-
consequential-control summary cross-referencing
`docs/decisions/ADR-PURSUIT-APPROVAL-BOUNDARY-V1.md`. Also corrected an
over-strong Brandeis degree-conferral wording to evidence-neutral
language: current evidence supports that all Brandeis MS Business
Analytics (STEM) program requirements are satisfied; **degree conferral
is NOT independently established** in current evidence and must not be
asserted or silently upgraded unless supported by updated evidence (the
41-vs-43-unit discrepancy remains separately unresolved, unaffected by
this sync). `AGENTS.md` gained a concise fresh-source operational rule,
an approved-Claim-is-not-an-automatic-capability-mapping standing rule,
and a priority-selection rule; `CLAUDE.md` gained two concise cross-
references, not a restatement. Independent Cursor adversarial review
(two rounds; first `REQUIRES_CORRECTION` on interpretive degree wording
and a fresh-source rule missing from `BLUEPRINT.md` itself, both
corrected): `SAFE_TO_COMMIT_REAL_WORLD_OPERATION_AND_CALIBRATION_
GOVERNANCE_SYNC`. **CANONICAL DIRECTION**: no new PRODUCT implementation
milestone is currently selected; **OPERATE FIRST, BUILD SECOND** is now
canonical -- new development is earned only by a demonstrated
reliability/truth defect or a meaningful Bora workload reduction, per the
build-economy gate. MIT LL gate migration, additional candidate-profile
heading synonyms, generic matcher cleanup, global NONE-vs-UNKNOWN
remediation, `DiscoveryLead` implementation, manual intake, dashboard,
crawler, ATS integrations, `ApplicationBrief`, outcome tooling,
networking tooling, package automation, and pursuit-approval
implementation each remain candidates only, not authorized by this sync.

JD Software Junior Project Manager alternative-qualification gate
(`JD_SOFTWARE_ALTERNATIVE_QUALIFICATION_GATE_APPLICATION_V1`) = **CLOSED**
(canonical merge commit `2c7caaf8be8f5cae7f7d205f1afd0576279f85c7`, PR
#4, parents `a6a86908c4ecf6056301db10a98f10708ed71fe1`/
`fd578b5bcbeee02808a3bd7274f413cdc37d284e`; hosted Assurance Baseline run
`33714772117` on the merge commit independently verified `success`).
Reproduced a live fabricated hard blocker on JD Software's Junior Project
Manager posting: `REQ_JDJPM_DEGREE` ("BS in a relevant discipline or, an
equivalent combination of education, training, and experience")
independently resolved `NONE` and hard-blocked despite the employer
explicitly permitting an alternative route. Fixed by reusing the already-
closed `ALTERNATIVE_QUALIFICATION_BRANCH_REPRESENTATION_V1`
`qualification_gate` architecture unchanged: new real fixture
`fixtures/jobs/CASE_F_JD_SOFTWARE_JPM_JUNIOR_PROJECT_MANAGER/` plus a new
bounded regression section in
`tests/alternative_qualification_branch_representation_v1_test.py`;
`GATE_JDJPM_DEGREE_COMPONENT` wraps `REQ_JDJPM_DEGREE` alone, the
unquantified "equivalent combination" branch preserved via
`unmodeled_branches_note`, no fabricated second leaf, zero `src/` or
schema changes. `REQ_JDJPM_PM_EXPERIENCE` remains the sole genuine
independent hard blocker; overall decision REJECT unchanged; zero
regression on CASE_D/CASE_E/CASE_C/Atominvest/all 15 golden fixtures.
Cursor verdict (post one M-01 `employment_type` correction):
`SAFE_TO_COMMIT_JD_SOFTWARE_GATE_APPLICATION`.

Candidate-profile heading semantic scope
(`CANDIDATE_PROFILE_HEADING_SEMANTIC_SCOPE_V1`) = **CLOSED** (canonical
merge commit `c8eda27cd2b9611ca18c95b1a0eb395755ae2855`, PR #5, parents
`2c7caaf8be8f5cae7f7d205f1afd0576279f85c7`/
`99d5e29f75cbccbc272d66e0ec141dd0e0241df3`; hosted Assurance Baseline run
`33774550732` independently verified `success`). Resolved the "What We're
Looking For" heading gap the prior `SOURCE_SEMANTIC_ROLE_THRESHOLD_
COUNTING_PARITY_V1` closure had explicitly deferred, after real external
first-party corroboration (VGS, Fullscript) and JD Software Implementation
Analyst row-by-row analysis. New internal, non-persisted
`CANDIDATE_PROFILE_HEADING` category in `src/requirement_source_role.py`
-- weaker than `REQUIREMENTS_HEADING`, V1-scoped to exactly "What We're
Looking For"/"What We Are Looking For" only; a duty-shaped row under this
heading still resolves `AMBIGUOUS`; "What You Bring"/"Who You Are"/"About
You" deliberately NOT recognized (external evidence showed these can
carry broad culture content). No new persisted `source_semantic_role`
enum value. Implementation surface exactly that one file plus 13 new
test-first cases (W-1 through W-13, including deliberate negative scope-
lock assertions) in `tests/source_semantic_role_qualification_view_v1_
test.py`. Zero regression on all real/golden fixtures. Cursor verdict:
`SAFE_TO_COMMIT_CANDIDATE_PROFILE_HEADING_SEMANTIC_SCOPE`.

JD Software Implementation Analyst alternative-qualification gate
(`JD_SOFTWARE_IA_ALTERNATIVE_QUALIFICATION_GATE_APPLICATION_V1`) =
**CLOSED** (canonical merge commit
`07321f12018980fcc2a392d5590a959fcf1941c1`, PR #6, parents
`c8eda27cd2b9611ca18c95b1a0eb395755ae2855`/
`7ebed2fa9494d3039f41b8528ddffe01edbc9c82`; hosted Assurance Baseline run
`33776919142` independently verified `success`). After the heading fix
above made `REQ_IA_DEGREE` reachable, a fresh live re-fetch of
`https://www.jdsoft.com/career-ia.html` reproduced the same compound-OR
degree fabrication already solved for the sibling JPM posting. Fixed by
reusing the accepted `CASE_F` single-leaf `ANY_OF` gate pattern exactly:
new real fixture `fixtures/jobs/CASE_G_JD_SOFTWARE_IA_IMPLEMENTATION_
ANALYST/` (freshly captured, byte-identical across two independent
fetches) plus a new bounded regression section (P1/P2) in the same
alternative-qualification test file; zero `src/` or schema changes.
`REQ_IA_ANALYTICAL` remains the sole genuine independent hard blocker;
overall decision REJECT unchanged; `REQ_IA_RDBMS_PLUS` remains PREFERRED/
non-blocking; zero regression on CASE_F/CASE_D/CASE_E/CASE_C/Atominvest/
all 15 golden fixtures. Cursor verdict:
`SAFE_TO_COMMIT_JD_SOFTWARE_IA_ALTERNATIVE_QUALIFICATION_GATE`.

MIT Lincoln Laboratory's own, analogous compound-OR degree/experience row
(`REQ_C_DEGREE_EXPERIENCE`) remains explicitly a SEPARATE, DEFERRED,
NOT-authorized item -- `ALTERNATIVE_QUALIFICATION_GATE_COVERAGE_
COMPLETION_ARCHITECTURE_V1` (read-only) found it entangled with four
other already-closed milestones' pinned regression anchors and a
purpose-built `degree_experience_duration_conjunction` capability tag,
requiring an explicit architecture decision before any implementation --
not earned by this continuity sync.
...
Pursuit Approval Boundary architecture (`PURSUIT_APPROVAL_BOUNDARY_ARCHITECTURE_V1`) = **ARCHITECTURE RECORDED / IMPLEMENTATION DEFERRED** (not implemented, not an active milestone). `PURSUIT_APPROVAL_BOUNDARY_ARCHITECTURE_AUDIT_V1` (read-only) completed with `ARCHITECTURE_CONTRACT_JUSTIFIED_IMPLEMENTATION_DEFERRED`: no existing field (`Job.decision`, `Job.lane`, `Job.application_status`, `Job.resume_version`, `Job.network_action`, `ApplicationAttempt.attempt_status`) means "Bora explicitly chose to pursue this specific opportunity," but no current production consumer creates a consequential defect from that gap, since no pursuit-gated downstream consumer (résumé/package generation for pursuit, application-route initiation, networking execution, submission) exists yet. Accepted ADR: `docs/decisions/ADR-PURSUIT-APPROVAL-BOUNDARY-V1.md` (committed, canonical; architecture SHA `6a6a1da8b3ed2ddf6c6cf86b708d74bf2e0e0644`; two independent Cursor adversarial reviews, second verdict `ACCEPT_ADR_DRAFT`, one residual exploratory/consequential-boundary scoping note adjudicated `ACCEPTABLE_ARCHITECTURE_DEFERRAL`). Locked: system recommendation (`Job.decision`/`Job.lane`, fully system-derived) never silently becomes Bora's human pursuit authorization; pursuit authorization is an opportunity-level human-authorization slice within the broader Pursuit truth layer (`BLUEPRINT.md` §132), distinct from Employer/Candidate/Match/posting-actionability truth, `Job.application_status`, `ApplicationAttempt`, Application Gate, résumé/package truth, networking-action authorization, submission authorization, and outcome truth; read-only/decision-support analysis (e.g. `analyze_job` re-analysis, exploratory application-route inspection) may occur before pursuit authorization when it does not materially advance pursuit; a materially changed opportunity must not silently inherit stale authorization merely because `Job_ID` is unchanged; the exact future representation/enforcement mechanism is explicitly deferred (no schema, enum, state machine, persistence, production code, tests, package generation, networking execution, application-readiness engine, or submission automation is authorized). No current runtime pursuit-authorization defect was reproduced by `PURSUIT_APPROVAL_BOUNDARY_ARCHITECTURE_AUDIT_V1`. No active implementation milestone exists for pursuit approval; implementation activates only when a concrete downstream pursuit consumer (résumé/package generation is the currently expected, not locked, first candidate) is separately earned, architected, reviewed, and explicitly authorized.

Main branch protection / merge enforcement governance record (not a product implementation milestone; supersedes the historical `BRANCH_PROTECTION_UNVERIFIED` text below for CURRENT branch-enforcement truth -- see chronology). `MAIN_BRANCH_PROTECTION_AND_MERGE_ENFORCEMENT_AUDIT_V1` (read-only) = `MINIMAL_ENFORCEMENT_ARCHITECTURE_JUSTIFIED`, independently re-verifying at its own audit baseline that `main` was unprotected, required-status-check enforcement was off, and repository rulesets were `[]`. `MAIN_BRANCH_PROTECTION_AND_MERGE_ENFORCEMENT_V1` = `SETTINGS_APPLIED_AND_VERIFIED`. Chronology: (1) at `REPRODUCIBLE_CONSEQUENTIAL_ASSURANCE_BASELINE_V1` closure (below), branch enforcement was genuinely `BRANCH_PROTECTION_UNVERIFIED` -- that milestone verified reproducible assurance only, not branch enforcement, and said so explicitly; (2) `MAIN_BRANCH_PROTECTION_AND_MERGE_ENFORCEMENT_AUDIT_V1` later superseded that epistemic state via direct, independent GitHub API re-verification of no enforcement; (3) `MAIN_BRANCH_PROTECTION_AND_MERGE_ENFORCEMENT_V1` then created and independently verified an active minimal repository ruleset -- CURRENT state: `main` is protected by that ruleset (GitHub's branch metadata now reports `protected: true` for exactly this reason), not by classic branch protection, which remains a separate, still-unused mechanism. Live enforced architecture (independently verified against the GitHub API): ruleset `main-minimal-enforcement`, ID `22154366`, target `refs/heads/main` only, enforcement `active`, bypass actors none, `current_user_can_bypass: never`; `main` deletion blocked; force-push/non-fast-forward blocked; pull request required with `0` required approving reviews (no CODEOWNERS review, no last-push approval, no review-thread-resolution requirement); required status check `verify`, source/integration GitHub Actions (`integration_id 15368`, independently confirmed against a live check run on commit `777423af7b190d36763abf605a7c6c3ff6adf5d0`); `strict_required_status_checks_policy: false` (loose mode -- the PR head must pass `verify`, need not be updated with latest `main` first). Intentionally not required: signed commits, CODEOWNERS, merge queue, linear history, GitHub-approving review, strict/up-to-date branch requirement -- each evaluated and omitted as unnecessary ceremony for a single-collaborator repository, closing no real additional risk. GitHub platform-populated field `require_extra_approval_for_unattributed_changes = true` governs unattributed Copilot pull requests (a Copilot-authored PR not opened on behalf of a person), not commit signatures or authorship verification, and is not an added human review gate; adjudicated `ACCEPTABLE_PLATFORM_DEFAULT_NO_SEMANTIC_EFFECT` because GitHub documents this field has no effect when `required_approving_review_count = 0`, which this ruleset sets. Bora retains final merge action and full settings/admin access, independent of the ruleset's empty bypass-actor list; admin ruleset edit/disable remains available for genuine emergency recovery, distinct from any routine merge-time bypass, of which none exists. `FORK_PR_CONTRIBUTOR_APPROVAL_POLICY = UNVERIFIED_REPO_SPECIFIC_VALUE` remains unmodified and uninferred (UI-only GitHub setting, not retrievable via REST/GraphQL in this environment) -- orthogonal to the ruleset: it governs whether an external contributor's fork PR workflow run may execute at all, while the ruleset governs whether `main` may be updated once a PR exists. New canonical development workflow (governance/process record, not a claim about GitHub's own enforcement capability): bounded branch -> independent Cursor adversarial review against the consequential uncommitted diff -> authorized commit to the bounded branch -> push the bounded branch -> pull request -> required GitHub Actions `verify` check -> Bora final merge -> `main`. GitHub mechanically enforces only PR association for changes to `main`, the required `verify` check and its bound source, no force push, and no `main` deletion -- GitHub does NOT mechanically verify that Cursor reviewed the diff, that any agent stayed in authorized scope, or that Bora's semantic adjudication was correct; those remain process/governance invariants. This record does not select, open, or imply a new PRODUCT implementation milestone; `CURRENT_MILESTONE.md` `Status: CLOSED` is unchanged, and `PURSUIT_APPROVAL_BOUNDARY_ARCHITECTURE_V1` remains ARCHITECTURE ACCEPTED / IMPLEMENTATION DEFERRED, unaffected.

Active milestone pointer: see `CURRENT_MILESTONE.md` (`Status: CLOSED`; closed task `REAL_WORLD_OPERATION_AND_CALIBRATION_GOVERNANCE_SYNC_V1`; no new active PRODUCT implementation milestone is currently selected; canonical direction is OPERATE FIRST, BUILD SECOND per `BLUEPRINT.md` §133's build-economy gate -- see the dated entries above for the full closure chain from this pointer back through `SOURCE_SEMANTIC_ROLE_THRESHOLD_COUNTING_PARITY_V1`). The "What We're Looking For" heading-cue gap this pointer previously named as the strongest next investigation candidate was itself resolved by the later `CANDIDATE_PROFILE_HEADING_SEMANTIC_SCOPE_V1` closure above -- that framing was accurate at its own recorded point in time and is preserved below as history, superseded only as current top-level state. `SOURCE_SEMANTIC_ROLE_THRESHOLD_COUNTING_PARITY_V1` = **CLOSED** (triggered by `POSITIVE_CONTROL_REAL_MARKET_FALSE_NEGATIVE_AUDIT_V1`'s reproduced live false negative on JD Software -- Implementation Analyst, adjudicated `POSITIVE_CONTROL_FALSE_NEGATIVE_DEFECT_JUSTIFIES_MILESTONE`; root defect: `decide_lane_and_decision()`'s `mandatory`/`preferred` threshold collections in `src/job_decision.py` did not apply the same `derive_qualification_gate(source_semantic_role) == "YES"` eligibility filter `detect_hard_blockers()` already applied, letting `ROLE_RESPONSIBILITY`/`APPLICATION_OR_LEGAL_GATE`/`AMBIGUOUS`/missing-role rows silently contaminate REJECT/WATCH/APPLY/PRIORITY_APPLY threshold counting with no trace in `hard_blockers`/`qualification_gaps`; fix: both collections now additionally require that filter, orthogonal to and alongside the pre-existing `gated_requirement_ids` exclusion; implementation surface exactly `src/job_decision.py` (14 lines) plus `tests/source_semantic_role_qualification_view_v1_test.py` (13 new test-first cases V-A through V-M, covering both negative and positive contamination, two mirroring a real live MIT LL instance); real-control result: JD Software's contaminated REJECT (`high_none=6`) became a truthful `UNDECIDED`/`UNASSIGNED`, not a fabricated APPLY; zero regression on Atominvest/MIT LL/MBTA Direct/MBTA Contractor/all 15 golden fixtures/Application Gate 9/9; Cursor verdict `SAFE_TO_COMMIT_SOURCE_SEMANTIC_ROLE_THRESHOLD_COUNTING_PARITY` (3 LOW observations accepted non-blocking); reviewed implementation commit `c76916df625b1ebaf2e29e7bbcb7bfc41799e3c7`, PR #2, merge commit `5ad657649b9df22eaa1e8a114380d05a36b08ed1` (parents `d1aa1144e4ae07d63300b2dfffdc6c0bd2d1af97`/`c76916df625b1ebaf2e29e7bbcb7bfc41799e3c7`), hosted Assurance Baseline run `33709096641` on the merge commit independently verified `success`; carried forward, not solved: "What We're Looking For" heading recognition, BS/B.S./BA degree-abbreviation parsing, global NONE-vs-UNKNOWN, candidate years-of-experience computation, technology-qualified duration, immigration/work-authorization inference, résumé/package generation, pursuit-approval implementation; see the dated `## Completed` entry below for full detail). `REPRODUCIBLE_CONSEQUENTIAL_ASSURANCE_BASELINE_V1` = **CLOSED** (architecture SHA `d8826aa368e5dbfafb80531f03913bd43cd00713`; authorization SHA `4a5ea68736f4c82274283ff154c7e53492320fba`; implementation SHA `b05c39022f791e3b6ef3f605f535a66620cd7c2a`; canonical ADR `docs/decisions/ADR-REPRODUCIBLE-CONSEQUENTIAL-ASSURANCE-BASELINE-V1.md`; selected after `POST_QUALIFICATION_GATE_REAL_MARKET_BOTTLENECK_AUDIT_V1` (`NO_IMPLEMENTATION_MILESTONE_JUSTIFIED`), `DEGREE_CREDENTIAL_CANDIDATE_EVIDENCE_ADJUDICATION_V1`, and `SYSTEM_WIDE_TRUST_AND_CONSISTENCY_AUDIT_V1` (both read-only complete) converged on assurance/reproducibility debt, not a reproduced business-logic defect, as the highest-earned next action; implementation bounded to `requirements.in`/`requirements-lock.txt`, `scripts/verify_assurance_baseline.py` (three-phase canonical command), `.github/workflows/assurance-baseline.yml` (SHA-pinned Actions), and `tests/p0_causal_invariants_v1_test.py` (P0 integration invariants A-E, including a Cursor M-01-corrected A2 high_none-counting isolation case); one prior ADR-phase `REQUIRES_CORRECTION` and one prior implementation-phase M-01 correction, both resolved and re-reviewed `SAFE_TO_COMMIT`; hosted GitHub Actions run (id `33678696541`, head SHA `b05c39022f791e3b6ef3f605f535a66620cd7c2a`, Ubuntu 24.04, Python 3.14.6) independently verified `ALL PHASES PASSED` (Phase 2 60/60, Application Gate 9/9, Job Analysis Golden 15/15); `REPRODUCIBILITY_UNVERIFIED` closed for this exact canonical baseline/hosted environment only -- not a claim that every OS/Python version is supported or that future dependency changes are automatically reproducible; `BRANCH_PROTECTION_UNVERIFIED` unchanged; no business/production/schema/Claim/Evidence/Experience/résumé semantics touched at any stage; see the dated `## Completed` entry below for full detail). `ALTERNATIVE_QUALIFICATION_BRANCH_REPRESENTATION_V1` = **CLOSED** (canonical implementation SHA `b10a38d09da60ef2e833fcbf718778e4132edabc`; architecture SHA `26f799075bd44c6fad729b4e14043c3eec2ab31c`; authorization/pointer SHA `beff126455ec933cad78dcdb30837148c525fea1`; canonical ADR `docs/decisions/ADR-ALTERNATIVE-QUALIFICATION-BRANCH-REPRESENTATION-V1.md`; see the dated `## Completed` entry below for full detail, including the mid-milestone CASE_E provenance correction). `DOMAIN_QUALIFIED_EXPERIENCE_DURATION_UNKNOWN_V1` = **CLOSED** (canonical implementation SHA `a4e849fe2629f4f25293e685776f49a1b1eddaa7`; authorization/pointer SHA `544125f2a6fbf466e40d4292313b05b974ada3ce`; selection baseline `dee032295cdfb95c79063c4179a2eb0b0a547c29`). Selected only after `POST_CLOSURE_REAL_JOB_SYSTEM_BOTTLENECK_AUDIT_V1` (read-only) reproduced the same epistemic hard-blocker defect on two independent real MBTA controls (`REQ_D_SYS_ANALYSIS_EXP`/`REQ_E_SYS_ANALYSIS_EXP`: `ENTRY_QUALIFICATION`, MANDATORY, `domain="System Analysis"`, empty inferred capabilities, NONE fabricated into a hard blocker with no domain or duration comparison actually performed) and `DOMAIN_QUALIFIED_EXPERIENCE_DURATION_SEMANTIC_SCOPE_V1` (read-only) designed a narrow correction: a new domain-qualified-duration evaluator (`src/domain_qualified_duration.py`, wired into `src/job_analysis.py`) routes a requirement only when domain is non-empty, technology is empty, `infer_requirement_capabilities()` (unmodified) returns nothing, and the text matches a bounded "N years of experience in <domain>" grammar -- always resolving `UNKNOWN`, never NONE/PARTIAL/SUPPORTED/STRONG, verified to exclude SAP/Salesforce/named-platform/UAT/technology-qualified/domain-free-generic cases (10-case regression matrix, all passing). Accepted real-control effect: `REQ_D_SYS_ANALYSIS_EXP`/`REQ_E_SYS_ANALYSIS_EXP` NONE -> UNKNOWN, no longer hard blockers; MBTA Direct remains REJECT with blocker `REQ_D_DEGREE` only; MBTA Contractor remains REJECT with blocker `REQ_E_DEGREE` only; fabricated `qualification_gaps` entries removed, truthful `qualification_unknowns` entries added; no APPLY-like decision introduced; Atominvest/MIT LL/BSA synthetic unchanged. Implementation surface: new `src/domain_qualified_duration.py` + `src/job_analysis.py` + `tests/domain_qualified_experience_duration_unknown_v1_test.py` (new, test-first) + 4 pre-existing tests corrected for stale CASE_D/E hard-blocker-set expectations (`accredited_institution_qualifier_semantics_v1_test.py`, `business_rules_technical_requirements_compound_completion_v1_test.py`, `process_mapping_compound_completion_v1_test.py`, `source_semantic_role_qualification_view_v1_test.py`); `requirement_match.py`, `experience_range.py`, `job_decision.py`, `requirement_source_role.py`, `requirement_normalize.py`, `application_gate.py`, schemas, Claims/Evidence/Experiences/résumé, and posting-state/immigration logic remained unchanged (verified zero diff). Independent Cursor adversarial review: `SAFE_TO_COMMIT_AND_PUSH` (no BLOCKING/HIGH/MEDIUM findings, two LOW documentation/test-wording observations only, not remediated in this closure); full suite `TOTAL=58 FAILED=0`; Job Analysis Golden 15/15; Application Gate Golden 9/9; implementation commit independently verified on GitHub by ChatGPT Work. Carried forward, not reopened: NONE-vs-UNKNOWN is NOT globally solved (remains a separate, secondary defect, now only narrowly addressed for domain-qualified duration requirements); do not wire approved MM/TELUS Claims merely because they are approved; no new Claim-to-capability mapping authorized; no candidate years-of-experience computation exists; technology-qualified duration remains outside this milestone; immigration/work-authorization logic remains separate and conservative; posting-state/Application Gate remain unchanged. `SOURCE_SEMANTIC_ROLE_QUALIFICATION_VIEW_V1` = **CLOSED** (canonical implementation SHA `ddc29b9525acee7de141cd9551d9f3b39665a718`, state-closure SHA `dee032295cdfb95c79063c4179a2eb0b0a547c29`; historical implementation baseline `e3af81a7ce6bd149eb2d0415bc7d1d217c600f61`) -- its accepted causal results and carried-forward exclusions (do not wire approved MM/TELUS Claims merely because they are approved; no new Claim-to-capability mapping authorized; no immigration/work-authorization inference added; "gain exposure to..." classifier edge behavior accepted) are unchanged and not reopened. `ATOMINVEST_REJECT_CAUSALITY_AND_APPLICATION_ACTIONABILITY_AUDIT_V1` remains `COMPLETE_ADJUDICATED` (baseline `e3af81a7ce6bd149eb2d0415bc7d1d217c600f61`). Prior pointers remain closed, not reopened: `ACCREDITED_INSTITUTION_QUALIFIER_SEMANTICS_V1` `CLOSED` (implementation SHA `9950c7c3eacdebf741c2e6a990a5b391adba3c44`, state-closure SHA `bf1f395ee1d79dec04f7ac39e3d972e48dcbe304`); `APPROVED_CLAIM_CAPABILITY_MAPPING_CAUSALITY_AUDIT_V1` remains `COMPLETE_ADJUDICATED` (baseline `01142d19fa80400ce94db5f5fa2e85ea01f23e1c`; adjudication result: do not wire MM/TELUS Claims yet, merely because they are approved).
...
* P-2 process-mapping evidence model (**CLOSED**):
  ...
  * No résumé-generation work begun.
  * Status: **CLOSED**.
* Alternative Qualification Branch Representation v1 (`ALTERNATIVE_QUALIFICATION_BRANCH_REPRESENTATION_V1`) (**CLOSED**):

  * Root cause: two live, currently-open real MBTA postings (CASE_D Job #26-20235, CASE_E Job #20260804A-ITS87) state that alternative education/experience branches satisfy one mandatory employer gate; flat, atomic Requirement rows and `job_decision.py`'s per-row hard-blocker loop could only represent the single Bachelor's-branch condition, and naively adding every branch as its own mandatory row would have fabricated false blockers.
  * Canonical ADR: `docs/decisions/ADR-ALTERNATIVE-QUALIFICATION-BRANCH-REPRESENTATION-V1.md` (commit `26f799075bd44c6fad729b4e14043c3eec2ab31c`) -- Option B: Requirement rows remain atomic; a new, separate `qualification_gate` Employer-truth record (additive `qualification_gates[]` in `structured_extraction.json`) composes them via a recursive `ALL_OF`/`ANY_OF` expression over Requirement IDs, carrying no candidate-specific state (static gate invariant).
  * New `src/qualification_gate.py`: a conservative, Match-truth-only, `evaluation_path`-keyed leaf policy (`SUPPORTED`/`BLOCKED_BY_MATCHING_POLICY`/`UNRESOLVED` -- never a claim about candidate factual reality); only the `NONE_TRAP` evaluation path may resolve `BLOCKED_BY_MATCHING_POLICY`, proven necessary because the identical capability signature can arise from both complete and incomplete compound requirements; reuses `application_logic.evaluate_expression()` completely unmodified as the deterministic tree walker.
  * Additive `evaluation_path` field on `EvidenceMatch` (`schemas/evidence_match.schema.json`), populated by `src/requirement_match.py`/`src/experience_range.py`/`src/domain_qualified_duration.py` at their own result-producing sites; new `schemas/qualification_gate.schema.json`; additive `qualification_gate_results` on `schemas/job_analysis_result.schema.json`.
  * `src/job_analysis.py` reads/validates (schema, fail-closed raw-`jd.txt`-traceability via exact-substring-after-whitespace-normalization, fail-closed referential integrity) and evaluates each gate; `src/job_decision.py` excludes gate-referenced rows from both the hard-blocker loop and the mandatory/preferred counting loop (the latter a necessary addition beyond the ADR's literal text). `src/requirement_normalize.py` needed no change.
  * **Mid-milestone BLOCKING correction** (found by Cursor's first adversarial review): the initial CASE_E authoring copied CASE_D's 4-branch, questionnaire-grounded system-analysis-duration structure (10/6/1 years) onto CASE_E merely because the postings are similar -- invalid, since CASE_E's own restored `jd.txt` states no equivalent explicit per-branch system-analysis-duration figures (only a single fixed "at least three (3) years" Yes/No question), unlike CASE_D whose own Q1 explicitly states them. CASE_E was corrected to a source-grounded "degree-component" gate (`GATE_E_DEGREE_COMPONENT`): the always-mandatory `REQ_E_SYS_ANALYSIS_EXP` `ALL_OF`'d with an `ANY_OF` over Bachelor's / (HS+GED + 7yr directly-related experience) / (Associate's + 3yr directly-related experience) -- duration legs correctly typed as generic "directly related experience" (`domain=null`), never mislabeled as system-analysis experience. Master's/certification substitutions remain unmodeled for CASE_E (the source text does not state what component they substitute for) and for CASE_D's certification branch, recorded via `unmodeled_branches_note` only, never invented. A permanent regression test checks semantic grounding (not merely `source_text`-substring presence) and fails closed on any future reintroduction.
  * Real-control effect: CASE_D gate resolves `UNRESOLVED` (all 8 branch leaves); decision remains REJECT via unrelated, unaffected ungrouped gaps (ITSM/SaaS/MS Office). CASE_E gate resolves `UNRESOLVED` (6 leaves); decision changes from REJECT to **UNDECIDED** -- an honest, unmanufactured consequence of removing the fabricated degree-only blocker, surfacing CASE_E's actual underlying state (one genuinely STRONG requirement, process mapping, alongside unrelated NONE gaps) under the existing, unmodified decision thresholds, which were not altered to avoid this outcome. No APPLY-like decision introduced either way; Atominvest and MIT LL unaffected.
  * Test-first: `tests/alternative_qualification_branch_representation_v1_test.py` (new) + 6 pre-existing tests corrected for stale CASE_D/E blocker-set/decision expectations (`accredited_institution_qualifier_semantics_v1_test.py`, `business_rules_technical_requirements_compound_completion_v1_test.py`, `domain_qualified_experience_duration_unknown_v1_test.py`, `process_mapping_compound_completion_v1_test.py`, `process_mapping_real_grammar_v1_test.py`, `source_semantic_role_qualification_view_v1_test.py`).
  * `requirement.schema.json`, `application_logic.py`, `requirement_source_role.py`, `requirement_normalize.py`, `application_gate.py`, all 15 golden fixtures, Atominvest, MIT LL, Claims/Evidence/Experiences/résumé, and posting-state/immigration logic remained unchanged (verified zero diff).
  * Two independent Cursor adversarial reviews: first `REQUIRES_CORRECTION` (the CASE_E provenance defect above), corrected; second `SAFE_TO_COMMIT_AND_PUSH`, no BLOCKING/HIGH/MEDIUM findings; implementation commit independently verified on GitHub by ChatGPT Work. Full suite `TOTAL=59 FAILED=0`; Job Analysis Golden 15/15; Application Gate Golden 9/9; posting-state wiring PASS.
  * Carried forward, not reopened: no global NONE-vs-UNKNOWN remediation; no Master's/Associate's base-credential capability-pattern expansion; no technology-qualified-duration fix; no Application Gate capture (real `ApplicationAttempt`/`ApplicationQuestion` authoring) for either MBTA role; no package-generation work; no certification-branch arithmetic invented for either fixture; no candidate years-of-experience computation exists anywhere in the pipeline.
  * No new active implementation milestone is currently selected; the next action is a fresh, read-only real-market/system bottleneck audit and prioritization.
  * Status: **CLOSED**.
* Reproducible Consequential Assurance Baseline v1 (`REPRODUCIBLE_CONSEQUENTIAL_ASSURANCE_BASELINE_V1`) (**CLOSED**):

  * Root cause: strong manually executed assurance (59 standalone test scripts, Job Analysis Golden 15/15, Application Gate Golden 9/9, posting-state/qualification-gate regressions, multiple Cursor adversarial reviews) was not yet reproducibly reconstructable from repo-declared dependencies, executable through one canonical command, automatically run in GitHub CI, or guaranteed to exercise the P0 causal integration invariants the two most recent architecture milestones introduced -- assurance debt (locked terminology `REPRODUCIBILITY_UNVERIFIED`, not `REPRODUCIBILITY_BROKEN`), not a reproduced business-logic defect. Selected after `POST_QUALIFICATION_GATE_REAL_MARKET_BOTTLENECK_AUDIT_V1` (`NO_IMPLEMENTATION_MILESTONE_JUSTIFIED`) and `DEGREE_CREDENTIAL_CANDIDATE_EVIDENCE_ADJUDICATION_V1`/`SYSTEM_WIDE_TRUST_AND_CONSISTENCY_AUDIT_V1` (both read-only) converged on it as the highest-earned next action.
  * Canonical ADR: `docs/decisions/ADR-REPRODUCIBLE-CONSEQUENTIAL-ASSURANCE-BASELINE-V1.md` (architecture SHA `d8826aa368e5dbfafb80531f03913bd43cd00713`, one prior `REQUIRES_CORRECTION` on the canonical verification command's ambiguous phase structure, corrected before Cursor's `SAFE_TO_COMMIT_ADR`).
  * Implementation bounded to exactly: `requirements.in` (direct deps: `jsonschema`, `referencing`) / `requirements-lock.txt` (complete resolved transitive set: `attrs`, `jsonschema`, `jsonschema-specifications`, `referencing`, `rpds-py`, exact versions); `scripts/verify_assurance_baseline.py` (one canonical command, three phases -- compile/syntax, sorted `tests/*_test.py` discovery/execution with fail-closed zero-discovery and mandatory-anchor checks, Job Analysis Golden runner; `scripts/generate_job_analysis_golden_fixtures.py` never executed); `.github/workflows/assurance-baseline.yml` (triggers on `push`/`pull_request` to main, least-privilege permissions, `actions/checkout`/`actions/setup-python` pinned to independently-verified full commit SHAs); `tests/p0_causal_invariants_v1_test.py` (new P0 integration tests A-E: gated-leaf exclusion from hard_blockers/gaps/unknowns and, after a Cursor M-01 narrow correction, from the separate mandatory/HIGH-NONE counting path inside `decide_lane_and_decision()` specifically; invalid trusted Claim/Evidence repository state fails closed; invalid gate requirement references/source provenance fail before consequential routing; Application Gate independence from qualification-gate result). No `.gitattributes`/OS matrix/packaging-tool conversion/hash-enforcement added (none independently justified).
  * M-01 (Cursor, implementation-phase): the original P0 §A only exercised a `BLOCKED_BY_MATCHING_POLICY` gate scenario, which exits `decide_lane_and_decision()` via `hard_blockers` before its separate `high_none`-counting branch is ever reached -- a regression isolated to that specific list-comprehension exclusion could have escaped detection. Corrected by adding P0 §A2: an `UNRESOLVED` (not `BLOCKED`) single-leaf gate scenario reaching the `high_none >= 1` branch genuinely, asserting `decision == "WATCH"` (not `REJECT`) and the exact `high_none` rationale text's absence, plus a comparable ungrouped control proving the identical requirement is causally significant (`REJECT`) when not gated -- proving the gate, not some other property, suppresses the effect. Second Cursor review: `SAFE_TO_COMMIT_AND_PUSH`, M-01 closed.
  * Local verification: `python scripts/verify_assurance_baseline.py` exit 0, all three phases PASS, Phase 2 60/60 (all mandatory anchors present). Independently re-verified in a genuinely isolated, freshly-downloaded Python 3.14.7 environment (official embeddable distribution, SHA256-checksum-verified against python.org) after an initial false-negative external-availability check was corrected by direct local parsing of `actions/python-versions`' `versions-manifest.json` (which showed exact Python 3.14.6 was, in fact, available all along) -- byte-for-byte identical output to the 3.14.6 run.
  * Hosted evidence (independently verified by ChatGPT Work): GitHub Actions "Assurance Baseline" run id `33678696541`, head SHA `b05c39022f791e3b6ef3f605f535a66620cd7c2a`, Ubuntu 24.04, Python 3.14.6, conclusion `success` -- checkout/setup-python SHA pins executed, dependencies reconstructed solely from `requirements-lock.txt`, canonical verifier ran, Phase 1/2 (60/60)/3 (Golden 15/15) all PASS, Application Gate 9/9 PASS.
  * `REPRODUCIBILITY_UNVERIFIED` is now closed for this exact canonical baseline and hosted environment only -- not a claim that every OS/Python version is supported, that branch protection exists, or that every future dependency change is automatically reproducible. `BRANCH_PROTECTION_UNVERIFIED` unchanged; CI existence/success does not prove merge enforcement.
  * No production/business/schema/Claim/Evidence/Experience/résumé semantics touched at any stage (implementation surface: exactly the 5 files above, verified by diff-tree at every commit).
  * No new active implementation milestone is currently selected; the next action is a fresh, read-only real-market/system bottleneck audit and prioritization.
  * Status: **CLOSED**.
* Pursuit Approval Boundary v1 (`PURSUIT_APPROVAL_BOUNDARY_ARCHITECTURE_V1`) (**ARCHITECTURE RECORDED / IMPLEMENTATION DEFERRED**):

  * `PURSUIT_APPROVAL_BOUNDARY_ARCHITECTURE_AUDIT_V1` (read-only) found no existing field represents Bora's explicit human decision to pursue a specific opportunity, and adjudicated `ARCHITECTURE_CONTRACT_JUSTIFIED_IMPLEMENTATION_DEFERRED`: a genuine semantic gap, but no reproducible current defect, since no pursuit-gated downstream consumer exists yet.
  * Accepted ADR: `docs/decisions/ADR-PURSUIT-APPROVAL-BOUNDARY-V1.md` (committed, canonical; architecture SHA `6a6a1da8b3ed2ddf6c6cf86b708d74bf2e0e0644`; two Cursor adversarial reviews, second verdict `ACCEPT_ADR_DRAFT`; one residual exploratory/consequential-boundary scoping note adjudicated `ACCEPTABLE_ARCHITECTURE_DEFERRAL`, not a contradiction).
  * Locks (semantics only, no schema/enum/state machine/code authorized): system recommendation (`Job.decision`/`Job.lane`) is not human pursuit authorization; pursuit authorization is an opportunity-level human-authorization slice within the broader Blueprint §132 Pursuit truth layer, distinct from every other truth surface (Employer/Candidate/Match/posting-actionability truth, `Job.application_status`, `Job.network_action`, `ApplicationAttempt`, Application Gate, résumé/package truth, submission authorization, outcome truth); read-only/decision-support analysis is permitted before pursuit authorization under a bounded safe harbor; stale human intent must not silently survive a materially changed opportunity; exact future representation/enforcement is explicitly deferred.
  * This is **not** a closed production implementation milestone: no schema, code, test, or persistence was authorized or created. `CURRENT_MILESTONE.md` `Status: CLOSED` (the assurance baseline was then the most recent closed implementation milestone) was unchanged at the time; no active implementation milestone existed for pursuit approval at that time.
* Main Branch Protection and Merge Enforcement v1 (`MAIN_BRANCH_PROTECTION_AND_MERGE_ENFORCEMENT_AUDIT_V1` / `MAIN_BRANCH_PROTECTION_AND_MERGE_ENFORCEMENT_V1`) (**GOVERNANCE SETTINGS APPLIED / NOT A PRODUCT IMPLEMENTATION MILESTONE**):

  * `MAIN_BRANCH_PROTECTION_AND_MERGE_ENFORCEMENT_AUDIT_V1` (read-only) independently re-verified via the GitHub API that `main` was unprotected, required-status-check enforcement was off, and repository rulesets were `[]`; adjudicated `MINIMAL_ENFORCEMENT_ARCHITECTURE_JUSTIFIED` after an adversarial consistency check (fork-PR approval behavior, Dependabot, same-repo-PR non-deadlock, admin recovery path independent of bypass actors, no unnecessary ceremony) found no contradiction.
  * `MAIN_BRANCH_PROTECTION_AND_MERGE_ENFORCEMENT_V1` created and independently verified one active repository ruleset, `main-minimal-enforcement` (ID `22154366`), targeting `refs/heads/main` only: deletion blocked, force-push/non-fast-forward blocked, PR required with `0` required approving reviews, required status check `verify` bound to GitHub Actions (`integration_id 15368`), loose (`strict_required_status_checks_policy: false`) mode, no bypass actors. Full detail (including the `require_extra_approval_for_unattributed_changes` Copilot-PR clarification and the fork-contributor-approval `UNVERIFIED_REPO_SPECIFIC_VALUE`) recorded in the Current Phase governance paragraph above.
  * Not a product implementation milestone: no schema, production code, test, Claim, Evidence, Experience, résumé, or ADR was created or changed by either the audit or the settings application. `CURRENT_MILESTONE.md` `Status: CLOSED` (the assurance baseline was then the most recent closed product implementation milestone) was unaffected at the time.
* Source Semantic Role Threshold Counting Parity v1 (`SOURCE_SEMANTIC_ROLE_THRESHOLD_COUNTING_PARITY_V1`) (**CLOSED**):

  * Triggering provenance: `POSITIVE_CONTROL_REAL_MARKET_FALSE_NEGATIVE_AUDIT_V1` (read-only) discovered and executed a set of live positive-control roles (JD Software Implementation Analyst, Atominvest Implementation Analyst, JD Software Junior PM) against the real `analyze_job()` path and reproduced a live false negative on JD Software Implementation Analyst; adjudicated `POSITIVE_CONTROL_FALSE_NEGATIVE_DEFECT_JUSTIFIES_MILESTONE`. `SOURCE_SEMANTIC_ROLE_THRESHOLD_COUNTING_PARITY_V1` (read-only architecture + test-design audit) independently re-verified the causal code path against `src/job_decision.py`/`src/requirement_source_role.py` directly (not merely trusted from the triggering audit) and adjudicated `IMPLEMENTATION_ARCHITECTURE_JUSTIFIED`.
  * Root defect: `decide_lane_and_decision()`'s own `mandatory`/`preferred` threshold-collection list comprehensions (`src/job_decision.py`, previously ~L375-388) filtered only by `importance`/`relevance`/`gated_requirement_ids` -- never by `derive_qualification_gate(source_semantic_role) == "YES"`, the exact eligibility filter `detect_hard_blockers()` already applied at its own per-row loop. A `ROLE_RESPONSIBILITY`/`APPLICATION_OR_LEGAL_GATE`/`AMBIGUOUS`/missing-role requirement could therefore silently contaminate `none`/`high_none`/`partial`/`strong_or_supported`/`high_strong`/`distinct_high_claims`/`material_preferred_missing`/`nonmaterial_preferred_missing` and thus REJECT/WATCH/EFFICIENT_APPLY/APPLY/PRIORITY_APPLY routing, with the real cause invisible in `hard_blockers`/`qualification_gaps` (both already correctly excluded such rows).
  * Fix: both list comprehensions gained `derive_qualification_gate(r.get("source_semantic_role")) == "YES"`, reusing the existing, unmodified function -- orthogonal to, and applied alongside, the pre-existing `gated_requirement_ids` exclusion. `src/job_decision.py` only (14 lines); no second eligibility definition created; `is_information_deficit()` deliberately left unmodified (responsibility rows remain valid JD-substance evidence, just not qualification-threshold-eligible).
  * Test-first: 13 new regression cases (`V-A` through `V-M`) added to `tests/source_semantic_role_qualification_view_v1_test.py`; `V-A` reproduced the pre-fix false REJECT (a lone `ROLE_RESPONSIBILITY` mandatory HIGH NONE row independently causing REJECT via `high_none=1`) on the unfixed baseline before any production edit. Coverage is symmetric: negative contamination (NONE rows fabricating REJECT: `V-A`/`V-C`/`V-D`/`V-E`/`V-J`/`V-K`/`V-L`) and positive contamination (STRONG/PARTIAL rows inflating positive counts or wrongly suppressing an earned PRIORITY_APPLY: `V-F`/`V-G`/`V-M`); genuine `ENTRY_QUALIFICATION` behavior unchanged (`V-B`/`V-H`); alternative-gate exclusion co-exists correctly with the new filter (`V-I`). `V-G`/`V-M` directly mirror a real, live instance found during the audit: `CASE_C_MIT_LL`'s `REQ_C_REQUIREMENTS_ANALYSIS` (`ROLE_RESPONSIBILITY`, matched `STRONG`) was already inflating positive counts on a real fixture, masked only because unrelated hard blockers fired first.
  * Real-control effect: JD Software Implementation Analyst's contaminated REJECT (`high_none=6`, half genuine post-hire duties) became a truthful `UNDECIDED`/`UNASSIGNED` (`hard_blockers=[]`, `qualification_gaps=[]`) -- every row now resolves either genuinely `ROLE_RESPONSIBILITY` or `AMBIGUOUS` (the separate, still-deferred "What We're Looking For" heading-cue gap). The milestone never required APPLY as its success criterion and did not produce one.
  * Non-regression (independently re-executed, zero flips): Atominvest, MIT Lincoln Laboratory, MBTA Direct, MBTA Contractor all identical to pre-fix; all 15 Job Analysis Golden fixtures unchanged; Application Gate Golden 9/9 unchanged; `python scripts/verify_assurance_baseline.py` exit 0 both pre-commit and, per hosted evidence, post-merge.
  * Independent Cursor adversarial review: `SAFE_TO_COMMIT_SOURCE_SEMANTIC_ROLE_THRESHOLD_COUNTING_PARITY`; 3 LOW observations (`V-L` family-fit isolation framing, `V-F` rationale-level discriminator, `V-H` redundancy) accepted non-blocking, not reopened for cleanup.
  * Provenance: reviewed implementation commit `c76916df625b1ebaf2e29e7bbcb7bfc41799e3c7`; PR `#2` (`https://github.com/bchaush/bora-employer-pipeline/pull/2`); merge commit `5ad657649b9df22eaa1e8a114380d05a36b08ed1` (parents `d1aa1144e4ae07d63300b2dfffdc6c0bd2d1af97` and `c76916df625b1ebaf2e29e7bbcb7bfc41799e3c7`, both independently verified); hosted GitHub Actions "Assurance Baseline" run `33709096641`, event `push`, head SHA = the merge commit, conclusion `success`, independently verified.
  * Carried forward, not reopened: "What We're Looking For" heading-cue recognition (`requirement_source_role.py`'s `_REQUIREMENTS_HEADING_CUES`); BS/B.S./BA degree-abbreviation parsing; global NONE-vs-UNKNOWN remediation; candidate years-of-experience computation; technology-qualified duration; immigration/work-authorization inference; résumé/package generation; pursuit-approval implementation.
  * No new implementation milestone is selected by this closure. The strongest next investigation candidate is a read-only semantic-scope audit of the "What We're Looking For" heading-cue gap (JD Software's own live posting contains genuine candidate-qualification content that currently remains `AMBIGUOUS`, not `ENTRY_QUALIFICATION`, purely because that heading synonym is unrecognized) -- this is explicitly not a statement that widening `_REQUIREMENTS_HEADING_CUES` is already authorized or correct; the heading is semantically overloaded across real job descriptions and requires investigation before any implementation.
  * Status: **CLOSED**.
* Domain-Qualified Experience Duration Unknown v1 (`DOMAIN_QUALIFIED_EXPERIENCE_DURATION_UNKNOWN_V1`) (**CLOSED**):

  * Root cause: `REQ_D_SYS_ANALYSIS_EXP`/`REQ_E_SYS_ANALYSIS_EXP` (real MBTA direct/contractor fixtures) are `ENTRY_QUALIFICATION`, MANDATORY, `domain="System Analysis"`, `technology=[]`; the generic experience-range evaluator correctly excludes domain-qualified rows, `infer_requirement_capabilities()` returns empty, so the ordinary matcher fell through to a fabricated NONE hard blocker with no domain or duration comparison actually performed.
  * New narrow evaluator `src/domain_qualified_duration.py`, wired from `src/job_analysis.py` as a third requirement-partition branch: routes a requirement only when domain is non-empty, technology is empty, `infer_requirement_capabilities()` (unmodified) returns nothing, and the text matches a bounded "N years of experience in `<domain>`" grammar; always resolves `UNKNOWN`, never NONE/PARTIAL/SUPPORTED/STRONG.
  * 10-case regression matrix proves SAP FI/CO, Salesforce, UAT, technology-qualified (Python), domain-without-duration, duration-without-domain, technology+domain-both-present, and malformed-grammar rows all remain correctly excluded and unrouted.
  * Real-control effect: `REQ_D_SYS_ANALYSIS_EXP`/`REQ_E_SYS_ANALYSIS_EXP` NONE -> UNKNOWN, no longer hard blockers; MBTA Direct remains REJECT (`REQ_D_DEGREE` only); MBTA Contractor remains REJECT (`REQ_E_DEGREE` only); fabricated `qualification_gaps` entries removed, truthful `qualification_unknowns` entries added; no APPLY-like decision introduced; Atominvest and MIT LL unchanged; the BSA synthetic fixture remained unchanged in milestone-relevant match semantics and contains no domain-qualified-duration row.
  * Test-first: `tests/domain_qualified_experience_duration_unknown_v1_test.py` (new, 18 lettered sections) written and run pre-fix to reproduce the defect (confirmed NONE) before the production wiring, then re-run post-fix (confirmed UNKNOWN end-to-end).
  * 4 pre-existing regression tests corrected for stale CASE_D/E 2-item hard-blocker-set expectations, now 1-item (`accredited_institution_qualifier_semantics_v1_test.py`, `business_rules_technical_requirements_compound_completion_v1_test.py`, `process_mapping_compound_completion_v1_test.py`, `source_semantic_role_qualification_view_v1_test.py`).
  * `requirement_match.py`, `experience_range.py`, `job_decision.py`, `requirement_source_role.py`, `requirement_normalize.py`, `application_gate.py`, schemas, Claims, Evidence, Experiences, résumé, immigration/work-authorization logic, and posting-state logic all confirmed unchanged (zero diff).
  * Independent Cursor adversarial review: `SAFE_TO_COMMIT_AND_PUSH` (no BLOCKING/HIGH/MEDIUM findings; two LOW documentation/test-wording observations only, not remediated in this closure); implementation commit independently verified on GitHub by ChatGPT Work.
  * Implementation commit `a4e849fe2629f4f25293e685776f49a1b1eddaa7`; authorization/pointer commit `544125f2a6fbf466e40d4292313b05b974ada3ce`; selection baseline `dee032295cdfb95c79063c4179a2eb0b0a547c29`.
  * Full suite `TOTAL=58 FAILED=0`; Job Analysis Golden 15/15; Application Gate Golden 9/9; posting-state wiring tests pass; `git diff --check` clean.
  * Carried forward, not reopened: NONE-vs-UNKNOWN is NOT globally solved (remains a separate, secondary defect); do not wire approved MM/TELUS Claims merely because they are approved; no new Claim-to-capability mapping authorized; no candidate years-of-experience computation exists anywhere in the pipeline; technology-qualified duration requirements remain outside this milestone; immigration/work-authorization logic remains separate and conservative; `SOURCE_SEMANTIC_ROLE_QUALIFICATION_VIEW_V1` remains closed; posting-state/Application Gate semantics unchanged.
  * No new active implementation milestone is currently selected; the next action is a fresh, truth-first, read-only real-job/system bottleneck audit and prioritization.
  * Status: **CLOSED**.
* Résumé Architecture v1 (`RESUME_ARCHITECTURE_V1`) (**CLOSED**):
