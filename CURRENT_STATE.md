...
Active milestone pointer: see `CURRENT_MILESTONE.md` (`Status: IMPLEMENTATION_AUTHORIZED`; active task `DOMAIN_QUALIFIED_EXPERIENCE_DURATION_UNKNOWN_V1`, canonical baseline `dee032295cdfb95c79063c4179a2eb0b0a547c29`). Selected only after `POST_CLOSURE_REAL_JOB_SYSTEM_BOTTLENECK_AUDIT_V1` (read-only) reproduced the same epistemic hard-blocker defect on two independent real MBTA controls (`REQ_D_SYS_ANALYSIS_EXP`/`REQ_E_SYS_ANALYSIS_EXP`: `ENTRY_QUALIFICATION`, MANDATORY, `domain="System Analysis"`, empty inferred capabilities, NONE fabricated into a hard blocker with no domain or duration comparison actually performed) and `DOMAIN_QUALIFIED_EXPERIENCE_DURATION_SEMANTIC_SCOPE_V1` (read-only) designed a narrow correction: a new domain-qualified-duration evaluator routes a requirement only when domain is non-empty, technology is empty, `infer_requirement_capabilities()` (unmodified) returns nothing, and the text matches a bounded "N years of experience in <domain>" grammar -- always resolving `UNKNOWN`, never NONE/PARTIAL/SUPPORTED/STRONG, explicitly excluding SAP/Salesforce/named-platform/UAT/technology-qualified/domain-free-generic cases. Expected effect: both MBTA rows become non-blocking UNKNOWN, but both fixtures remain REJECT (the degree requirement remains an independent blocker under current approved evidence) -- no APPLY-like decision is expected. Implementation surface: new `src/domain_qualified_duration.py` + `src/job_analysis.py` + focused tests only; `requirement_match.py`, `experience_range.py`, `job_decision.py`, `requirement_source_role.py`, `requirement_normalize.py`, `application_gate.py`, schemas, Claims/Evidence/Experiences/résumé, and posting-state/immigration logic must remain unchanged absent a newly reproduced, separately adjudicated necessity. `SOURCE_SEMANTIC_ROLE_QUALIFICATION_VIEW_V1` = **CLOSED** (canonical implementation SHA `ddc29b9525acee7de141cd9551d9f3b39665a718`, state-closure SHA `dee032295cdfb95c79063c4179a2eb0b0a547c29`; historical implementation baseline `e3af81a7ce6bd149eb2d0415bc7d1d217c600f61`) -- its accepted causal results and carried-forward exclusions (NONE-vs-UNKNOWN remains a separate, secondary, not globally fixed defect; do not wire approved MM/TELUS Claims merely because they are approved; no new Claim-to-capability mapping authorized; no immigration/work-authorization inference added; "gain exposure to..." classifier edge behavior accepted) are unchanged and not reopened by the new active milestone. `ATOMINVEST_REJECT_CAUSALITY_AND_APPLICATION_ACTIONABILITY_AUDIT_V1` remains `COMPLETE_ADJUDICATED` (baseline `e3af81a7ce6bd149eb2d0415bc7d1d217c600f61`). Prior pointers remain closed, not reopened: `ACCREDITED_INSTITUTION_QUALIFIER_SEMANTICS_V1` `CLOSED` (implementation SHA `9950c7c3eacdebf741c2e6a990a5b391adba3c44`, state-closure SHA `bf1f395ee1d79dec04f7ac39e3d972e48dcbe304`); `APPROVED_CLAIM_CAPABILITY_MAPPING_CAUSALITY_AUDIT_V1` remains `COMPLETE_ADJUDICATED` (baseline `01142d19fa80400ce94db5f5fa2e85ea01f23e1c`; adjudication result: do not wire MM/TELUS Claims yet, merely because they are approved).
