...
Before any Career OS eval, harness, orchestration, or automation work, follow the canonical recovery order fixed in `docs/decisions/ADR-CAREER-OS-EVAL-HARNESS-SEQUENCE-V1.md` Section 6 (`main` -> `project_state.json` -> `CURRENT_EXECUTION_CHECKPOINT.json` -> `AGENTS.md`/`BLUEPRINT.md` -> this ADR/`CURRENT_MILESTONE.md`/`CURRENT_STATE.md`) — do not restate a different or partial order here. Phase B (`CAREER_OS_EVAL_AND_HARNESS_AUDIT_V1`) is COMPLETED_BY_OPERATOR and BORA_ACCEPTED; Phase C (`CAREER_OS_TRACE_AND_FAILURE_CORPUS_INVENTORY_V1`), READ_ONLY, is COMPLETED_BY_OPERATOR and BORA_ACCEPTED — see `docs/audits/CAREER_OS_TRACE_AND_FAILURE_CORPUS_INVENTORY_V1_REPORT.md` and `CURRENT_EXECUTION_CHECKPOINT.json`. The governance-only `CAREER_OS_AGENT_CONTEXT_AND_USAGE_EFFICIENCY_V1` policy milestone is COMPLETED_BY_OPERATOR and PENDING_BORA_ACCEPTANCE — see `docs/decisions/ADR-CAREER-OS-AGENT-CONTEXT-USAGE-EFFICIENCY-V1.md`. Phase D and every later phase remain PROPOSED_NOT_AUTHORIZED; do not jump directly to implementation, to trace infrastructure, or to Phase D merely because Phase C or this policy milestone's operator work is complete — Bora must separately accept each checkpoint and separately authorize a next phase before any later phase or implementation is authorized. The locked execution roles are ChatGPT architect/adjudicator/initiator -> Claude Code bounded builder after explicit authorization -> deterministic checks -> Cursor independent adversarial review -> CI -> Bora final merge/release authority.

## Agent Context & Usage Efficiency
Operational pointer: `docs/decisions/ADR-CAREER-OS-AGENT-CONTEXT-USAGE-EFFICIENCY-V1.md` is a candidate policy (COMPLETED_BY_OPERATOR / PENDING_BORA_ACCEPTANCE / NOT_YET_GOVERNING) proposing quality-preserving Claude Code / Cursor session and context discipline (bounded milestones, smallest sufficient relevant context, deterministic checks over agent reasoning where possible, Cursor as a consequential adversarial reviewer rather than an exploratory assistant). The operator cannot self-grant acceptance of this policy. If and when Bora accepts it, it will govern usage/context economy only — it will never override the Authority Order above, a locked rule, a validation schema, required review effort on consequential work, or any existing truth/evidence/approval boundary. Any structural change to `CLAUDE.md`, `.cursor/rules/*`, `.cursorignore`, or `.claude/` settings remains a separate, not-yet-authorized decision.
