...
assert_true(ev["valid"] is True and ev["records_checked"] == 13, "Evidence regression")
assert_true(
...
assert_true(
    ...
    "CLAIM_WW_006 must await reapproval after wording correction",
)
...
# Process mapping vocabulary recognized but NONE until CLAIM_WW_006 reapproved
...
assert_true(pmap["result"] == "NONE", f"P2 pending reapproval: {pmap}")
assert_true(not pmap.get("claim_ids"), pmap)
print("PASS R16: process mapping NONE pending CLAIM_WW_006 reapproval.")
...
assert_true(ww_claim["human_approval"] is False, "CLAIM_WW_006 must await reapproval")
...
print("PASS P2a: corrected CLAIM_WW_006 wording; pending reapproval.")
...
for text in [
    ...
    assert_true(m["result"] == "NONE", f"pending reapproval: {text!r} -> {m}")
    assert_true(not m.get("claim_ids"), m)
print("PASS P2b: process_mapping dormant until reapproval.")
...
print("PASS P2d: GT_PROCESS_MAP_P2 REJECT pending CLAIM_WW_006 reapproval.")
