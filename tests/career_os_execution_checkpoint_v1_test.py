...
assert 'COMPLETED_BY_OPERATOR / PENDING_BORA_ACCEPTANCE / READ_ONLY_DESIGN_ONLY' in cp['exact_next_allowed_action']
assert 'Operator completion is not Bora acceptance' in cp['exact_next_allowed_action']
assert 'review and accept or reject the Phase D checkpoint' in cp['exact_next_allowed_action']
...
# Phase D operator-completion event (a genuine substantive-work event, not
# Bora acceptance), while still stating implementation and Phase E are not
# authorized.
...
# the durable report path, without claiming Bora acceptance.
...
assert 'PENDING_BORA_ACCEPTANCE' in not_done
...
assert 'COMPLETED_BY_OPERATOR / PENDING_BORA_ACCEPTANCE' in PROJECT_STATE['next_authorized_action']
