...
CANONICAL_BASELINE_SHA = '2ce84321e8095b622328c77ea348b8f1fe874f5c'
...
# operator completion -- operator completion, Bora acceptance, and
# next-phase authorization must never collapse into one undifferentiated
# claim.
...
assert 'Bora reviews and accepts or rejects the Phase C' in cp['exact_next_allowed_action']

# terminal_adjudication must reflect Bora's acceptance of Phase B and
# operator completion (not Bora acceptance) of Phase C, while still
# stating Phase D is not authorized and implementation is not authorized.
...
# Must record genuine Phase C completed inventory actions, not merely an
# authorization-only transition, but must not fabricate Phase D work.
completed = ' '.join(cp['completed_actions'])
assert 'inventory' in completed.lower()
assert 'docs/audits/CAREER_OS_TRACE_AND_FAILURE_CORPUS_INVENTORY_V1_REPORT.md' in completed
...
assert PROJECT_STATE['current_phase'].startswith('CAREER_OS_TRACE_AND_FAILURE_CORPUS_INVENTORY_V1 (COMPLETED_BY_OPERATOR')
